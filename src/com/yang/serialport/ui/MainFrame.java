...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		//added by biggee
		for(int i=0;i<mCommChoice.getItemCount();i++) {
			if(mCommChoice.getItemAt(i).toString().equals("COM3")) {
				mCommChoice.setSelectedIndex(i);
				break;
			}
		}
		
		openSerialPort_noEvt();
		
		mDataInput.setText("他妈的终于能发送了");
		...
		closeSerialPort_noEvt();
		
		/*
		 * 任务：
		 * 需要相关方法：
		 * 
		 * public boolean openComPort(String) "COM1,9600,8N1"
		 * public boolean closeComPort_noEvt(String) "COM1"
		 * public boolean sendData_noEvt(String) "data"
		 * 
		 */
	}
	...
	private void mCommChoice_addPopupMenuListener_PopupMenuListener() {
		mCommList = SerialPortManager.findPorts();
		// 检查是否有可用串口，有则加入选项中
		if (mCommList == null || mCommList.size() < 1) {
			ShowUtils.warningMessage("没有搜索到有效串口！");
			...
			for (String s : mCommList) {
				mCommChoice.addItem(s);mCommChoice.getItemAt(index).toString();
			}
	...
	private void actionListener() {
		...
		mCommChoice.addPopupMenuListener(new PopupMenuListener() {
			...
			public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
				mCommChoice_addPopupMenuListener_PopupMenuListener();
			}
	...
	private void openSerialPort_noEvt() {
		// 获取串口名称
		...
		if (commName == null || commName.equals("")) {
			...
			try {
				...
				if (mSerialport != null) {
					mDataView.setText(commName + "串口已打开\r\n欢迎使用\r\n");
	 ...
	 *            点击事件
	 */
	private void closeSerialPort_noEvt() {
		String commName = (String) mCommChoice.getSelectedItem();
